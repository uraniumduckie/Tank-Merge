...
async function loadDashboard() {
    ...
    const { stats, users, totalUnlocks } = result.data;
    ...
    document.getElementById('statTotalUnlocks').textContent = stats.totalUnlocks ?? 0;
...
function renderUsersTable(users) {
    ...
    for (const u of users) {
        ...
        tr.innerHTML = `
            ...
            <td>${(() => { try { return JSON.parse(u.unlocked_tanks || '[]').length; } catch { return 0; } })()}</td>
            <td>${formatPlayTime(u.total_play_time || 0)}</td>
            <td style="white-space:nowrap;">
                <button class="viewUserBtn" data-userid="${u.id}">View</button>
                <button class="actionBtn editUserBtn" data-userid="${u.id}">Edit</button>
    ...
    tbody.querySelectorAll('.editUserBtn').forEach(b => b.addEventListener('click', () => openEditDialog(b.dataset.userid)));
...
async function confirmReset(userId, username) {
    const ok = await showConfirm('Reset Progress', `Reset all game progress for "${username}"? Events will also be cleared. Play time will be preserved.`);
    if (!ok) return;
    ...
    if (result.ok) {
        ...
        if (idx !== -1) {
            allUsers[idx].spawn_count = p.spawn_count;
            allUsers[idx].total_merges = p.total_merges;
            allUsers[idx].total_kills = p.total_kills;
            allUsers[idx].highest_tier = p.highest_tier;
            allUsers[idx].admin_mode = p.admin_mode;
            allUsers[idx].unlocked_tanks = p.unlocked_tanks;
        }
        applyFilter();
    }
}
...
function openEditDialog(userId) {
    ...
    if (!u) return;
    document.getElementById('editDialogTitle').textContent = `Edit: ${u.username || '(guest)'}`;
    document.getElementById('editUsername').value = u.username || '';
    document.getElementById('editIsAdmin').checked = !!u.is_admin;
    document.getElementById('editSpawns').value = u.spawn_count ?? 0;
    document.getElementById('editMerges').value = u.total_merges ?? 0;
    document.getElementById('editKills').value = u.total_kills ?? 0;
    document.getElementById('editHighestTier').value = u.highest_tier ?? 1;
    document.getElementById('editPlayTime').value = u.total_play_time ?? 0;
    document.getElementById('editAdminMode').checked = !!u.admin_mode;
    document.getElementById('editUnlockedTanks').value = u.unlocked_tanks || '[]';
...
async function saveEditDialog() {
    ...
    let unlockedRaw = document.getElementById('editUnlockedTanks').value.trim();
    try {
        if (unlockedRaw) JSON.parse(unlockedRaw);
    } catch {
        errorEl.textContent = 'Invalid JSON in unlocked tanks field.';
    ...
    const payload = {
        ...
        spawn_count: parseInt(document.getElementById('editSpawns').value) || 0,
        total_merges: parseInt(document.getElementById('editMerges').value) || 0,
        total_kills: parseInt(document.getElementById('editKills').value) || 0,
        highest_tier: parseInt(document.getElementById('editHighestTier').value) || 1,
        total_play_time: parseInt(document.getElementById('editPlayTime').value) || 0,
        admin_mode: document.getElementById('editAdminMode').checked ? 1 : 0,
        unlocked_tanks: unlockedRaw || '[]'
    };
    const result = await apiFetch(`/api/admin/users/${editUserId}`, { method: 'PUT', body: JSON.stringify(payload) });
    if (result.ok) {
        closeEditDialog();
        location.reload();
    } else {
        errorEl.textContent = result.data?.error || 'Failed to save.';
        errorEl.classList.remove('hidden');
    }
}
...
let refreshInterval = setInterval(() => {
    ...
    if (!detailSection.classList.contains('hidden')) return;
    refreshDashboard();
}, 5000);
